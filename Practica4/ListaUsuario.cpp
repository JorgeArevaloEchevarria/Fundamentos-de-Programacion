...
void inicializar(tListaUsuarios &usuarios){
	...
	usuarios.cont = 0;//inicializa la lista
...
bool insertar(tListaUsuarios &usuarios, const tUsuario &usuario){
	...
	if (usuarios.cont == MAX_USUARIOS)
		...
		while ((i > 0) && (usuario.id < usuarios.usuario[i - 1].id)){//busca al usuario a traves del id

			usuarios.usuario[i] = usuarios.usuario[i - 1];
			i--;

		}
		usuarios.usuario[i] = usuario;
		usuarios.cont++;
	}
	return ok;
...
bool cargar(tListaUsuarios &usuarios, string dominio){
	...
	flujo << dominio << ARCHIVOU;//nombre del archivo
	arch = flujo.str();
	...
	if (!archivo.is_open())
		...
		while (usuarios.cont <= MAX_USUARIOS && correcto){
			...
			correcto = cargar(usuarios.usuario[usuarios.cont], archivo);//carga los usuarios
			if (correcto)
				insertar(usuarios, usuarios.usuario[usuarios.cont]);
...
void guardar(const tListaUsuarios &usuarios, string dominio){
	...
	for (int i = 0; i < usuarios.cont; i++)
		guardar(usuarios.usuario[i], archivo);//guarda los usuarios

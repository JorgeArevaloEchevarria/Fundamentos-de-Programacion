...
bool arrancar(tGestor& gestor, string dominio){
	...
	if(ok)//si hemos podido cargar los usuarios cargaremos los correos sino ya devolveremos el error
...
void enviarCorreo(tGestor& gestor, const tCorreo &correo){
	...
	if(gestor.correos.cont < MAX_CORREOS && encontrado){
		...
		if(ok){
			...
			insertar(gestor.usuarios.usuario[gestor.usuarioActivo].enviados,registroEmisor);	//problema al mostrar los mensajes de la bandeja de salida

			if(gestor.usuarios.usuario[pos].recibidos.cont < MAX_REGS)
